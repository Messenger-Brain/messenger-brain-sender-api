...
export class UserController {
		...
		try {
			...
			const filters = {
				...
				status_id: req.query.status_id
					...
					: undefined,
				free_trial: req.query.free_trial
					? req.query.free_trial === "true"
					: undefined,
			};
	...
	public updateProfile = async (req: Request, res: Response): Promise<void> => {
		try {
			...
			if (result.success) {
				...
				this.logger.warn("Profile update failed", {
					userId,
					error: result.error,
				});
				res.status(400).json(result);
	 ...
	 */
	public requestDeleteProfile = async (
		req: Request,
		res: Response
	): Promise<void> => {
		try {
			...
			if (result.success) {
				...
				this.logger.warn("Profile deletion request failed", {
					userId,
					error: result.error,
				});
				if (
					...
					result.message.toLowerCase().includes("administrador")
				) {
	 ...
	 */
	public confirmDeleteProfile = async (
		req: Request,
		res: Response
	): Promise<void> => {
		try {
			...
			const result = await this.userService.confirmDeleteProfile(
				userId,
				token,
				password,
				confirmation
			...
			if (result.success) {
				...
				this.logger.warn("Profile deletion confirmation failed", {
					userId,
					error: result.error,
				});
				res.status(400).json(result);
	...
	public getUserActivities = async (
		...
		try {
			...
			const result = {
				success: true,
				data: { activities: [], total: 0, totalPages: 0, currentPage: page },
			};
	...
	public getValidationMiddleware() {
		return {
			...
			getUserById: this.validationMiddleware.validateParams(
				this.getUserByIdSchema
			),
			assignRole: this.validationMiddleware.validateBody(this.assignRoleSchema),
			removeRole: this.validationMiddleware.validateParams(
				this.removeRoleSchema
			),
		};
	}
