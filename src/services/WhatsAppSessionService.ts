...
export interface WhatsAppSessionServiceInterface {
	createSession(
		sessionData: CreateWhatsAppSessionRequest
	...
	updateSession(
		sessionId: number,
		user_id: number,
		sessionData: UpdateWhatsAppSessionRequest
	): Promise<ApiResponse<WhatsAppSession>>;
...
export class WhatsAppSessionService implements WhatsAppSessionServiceInterface {
	...
	public async createSession(
		...
		try {
			...
			const createData: any = {
				...
				phone_number: sessionData.phoneNumber,
				status_id: statusId,
				account_protection: sessionData.accountProtection ?? true,
				log_messages: sessionData.logMessages ?? true,
				webhook_enabled: sessionData.webhookEnabled ?? false,
			};
			...
			this.logger.info("WhatsApp session created successfully", {
	...
	public async updateSession(
		...
		try {
			...
			// Validate ownership
			const ownershipValid = await this.validateSessionOwnership(
				sessionId,
				user_id
			);
			if (!ownershipValid) {
			...
			if (
				...
				!this.isValidPhoneNumber(sessionData.phoneNumber)
			) {
				return {
					success: false,
					message: "Invalid phone number format",
			...
			await session.update({
				...
				...(sessionData.statusId !== undefined && {
					status_id: sessionData.statusId,
				}),
				...(sessionData.accountProtection !== undefined && {
					account_protection: sessionData.accountProtection,
				}),
				...(sessionData.logMessages !== undefined && {
					log_messages: sessionData.logMessages,
				}),
				...(sessionData.webhookUrl !== undefined && {
					webhook_url: sessionData.webhookUrl,
				}),
	...
	public async deleteSession(
		...
		try {
			this.logger.info("Deleting WhatsApp session", {
				sessionId,
				user_id: user_id,
			});
	...
	public async connectSession(
		...
		try {
			this.logger.info("Connecting WhatsApp session", {
				sessionId,
				user_id: user_id,
			});
	 ...
	 */
	public async getSessionsByStatus(
		statusSlug: string,
		pagination: PaginationQuery = {}
	): Promise<PaginatedResponse<WhatsAppSession>> {
		try {
	 ...
	 */
	public async getSessionCountByUser(
		user_id: number
